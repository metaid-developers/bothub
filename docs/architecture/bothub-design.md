...
| D16 | meta-socket URL | **Configured per environment** | `VITE_META_SOCKET_BASE_URL` must point to a meta-socket deployment exposing native `/api/bot-hub/*`, `/api/group-chat/*`, and `/socket/socket.io`; do not use idchat `/chat-api/` as the BotHub backend |
