...
公网 **技能服务广场（Bot Hub）+ 用户侧交付会话（Delivery）**：给普通 caller 用户使用的纯前端下单工具。用户用 **Metalet** 登录，浏览远端 provider bot 发布的 skill-service，输入自然语言需求并支付/发单；Delivery 通过 **metaso-p2p** 只监听当前用户 simplemsg，渲染执行过程和数字成果。
...
- **[下一阶段产品化设计](./docs/architecture/buyer-productization-design.md)** — caller/buyer 侧上线产品规格、Delivery 数字成果管理、IndexedDB、metaso-p2p 契约
- **[下一阶段实施计划](./docs/superpowers/plans/2026-05-28-buyer-productization.md)** — P0–P4 subagent-driven implementation plan
- **[Buyer 产品化验收清单](./docs/qa/buyer-productization-acceptance.md)** — 本地 metaso-p2p、Chrome + Metalet、Delivery 资产验收和 Run Log
...
- [聚合接口契约（历史草稿；权威 spec 在 metaso-p2p）](./docs/architecture/aggregator-contract.md)
...
- **P0 Release foundation**：构建、metaso-p2p API/socket 边界、纯静态部署说明。
...
- **P4 History sync & release cut**：metaso-p2p 私聊历史 + Socket.IO 推送合并去重，刷新后可恢复。
...
默认本机 metaso-p2p 为 `http://127.0.0.1:18091`。浏览器联调推荐走 Vite 同源代理 `/metaso-p2p`，避免本地 CORS 预检问题；将以下配置写入 `.env.local` 或 `.env`：
...
VITE_METASO_P2P_BASE_URL=https://so.metaid.io VITE_USE_AGGREGATOR_MOCK=false VITE_USE_WS_MOCK=false pnpm dev -- --host 127.0.0.1
```
...
- 本地/private beta 推荐：`VITE_METASO_P2P_BASE_URL=/metaso-p2p`、`VITE_USE_AGGREGATOR_MOCK=false`、`VITE_USE_WS_MOCK=false`，由 Vite proxy 转发到 `http://127.0.0.1:18091`。
- 生产/预发 metaso-p2p：当前已验证公网根域名为 `https://so.metaid.io`。`VITE_METASO_P2P_BASE_URL` 必须是 metaso-p2p 根域名，并直接提供 `/api/bot-hub/*`、canonical `/api/private-chat/*`、`/socket/socket.io`；不要指向 idchat `/chat-api/`。旧 `/api/group-chat/*` 私聊路由只作为兼容 fallback，不是新部署契约。
